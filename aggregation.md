...
只能用在最后一个stage。将结果导出到同一database下面的指定collection中（注意会完全覆盖），但是不清除原有index。

注意`$unwind`的结果不能写入，因为其中有重复文档。

## $merge

`$out`不能合并collection，于是merge来了。

* 可以跨数据库
* 指定要识别的id filds（用于识别是否为相同文档）
* 指定丰富的“当匹配”和“没有匹配”时的动作
